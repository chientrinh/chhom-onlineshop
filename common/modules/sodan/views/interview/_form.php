 ...
 * @version $Id: _form.php 4149 2019-04-04 07:27:55Z mori $
 *
...
$ajaxUrl = Url::to(['fetch-products']);
$ajaxUrl2 = Url::to(['fetch-homoeopath']);
$ajaxUrl3 = Url::to(['fetch-ticket']);
$jscode = "
$('select[name=\'Interview[client_id]\'], #interview-itv_date').change(function(){
  var client_id = $('#interview-client_id').val();
...
$('select[name=\'Interview[product_id]\']').children().each(function(){
   productArray.push( { value:$(this).val(), body:$(this).html() });
});
...
$('#product-filter').keyup(function(){
    option_filter($(this).val(), productArray, 'product_id');
});
";
...
if($model->client)
{
    ...
    elseif($client->getInterviews()->active()->exists()) {
        ...
        $age = $model->client->getAge($model->itv_date);
        if(null === $age) { }
...
<div class="interview-form">
    ...
    <div class="col-md-6">
        <div class="row">
            <?= $form->field($model, 'open_flg')->checkbox() ?>
        </div>
    <div class="row">
