<?php

namespace common\modules\sodan\controllers;

use Yii;
use yii\data\ActiveDataProvider;
use yii\web\NotFoundHttpException;
use common\models\Product;
use common\models\ProductSubcategory;


/**
 * 健康相談クーポン管理コントローラー
 */
class CouponController extends \yii\web\Controller
{
    public function beforeAction($action)
    {
        if(! parent::beforeAction($action))
            return false;

        $this->view->params['breadcrumbs'][] = ['label' => '健康相談クーポン', 'url' => ['index']];
        return true;
    }

    /**
     * Lists coupon product models.
     * @return mixed
     */
    public function actionIndex()
    {
        $param = Yii::$app->request->queryParams;
        $model = new Product();
        $model->load($param);
        $provider = new ActiveDataProvider([
            'query' => Product::find()->sodanCoupon()->andFilterWhere($model->dirtyAttributes)
        ]);

        return $this->render('index', [
            'dataProvider' => $provider,
            'searchModel'  => $model,
        ]);
    }

    /**
     * Displays a single BookTemplate model.
     * @param integer $id
     * @return mixed
     */
    public function actionView($id)
    {
        return $this->render('view', [
            'model' => $this->findModel($id),
        ]);
    }

    /**
     * Creates a new BookTemplate model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate()
    {
        $model = new Product([
            'category_id' => \common\models\Category::SODAN,
            'restrict_id' => \common\models\ProductRestriction::PKEY_INSTORE_ONLY,
            'in_stock' => 1 // 在庫OK
        ]);
        $model->applyDefaultValues();

        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            // サブカテゴリ登録
            $sub_model = new ProductSubcategory([
                'subcategory_id' => \common\models\Subcategory::PKEY_SODAN_COUPON,
                'ean13'          => $model->barcode
            ]);
            $sub_model->save();
            return $this->redirect(['view', 'id' => $model->product_id]);
        }

        return $this->render('create', [
            'model' => $model,
        ]);
    }

    /**
     * Updates an existing WaitList model.
     * If update is successful, the browser will be redirected to the 'view' page.
     * @param integer $id
     * @return mixed
     */
    public function actionUpdate($id)
    {
        $model = $this->findModel($id);

        if ($model->load(Yii::$app->request->post()) && $model->save())
            return $this->redirect(['view', 'id' => $model->product_id]);

        return $this->render('update', [
            'model' => $model,
        ]);
    }

    /**
     * Finds the BookTemplate model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return WaitList the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        $model = Product::findOne($id);
        if(! $model)
            throw new \yii\web\NotFoundHttpException('The requested page does not exist.');

        return $model;
    }
}
