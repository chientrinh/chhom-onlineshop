...
class DeliveryDateTimeForm extends \yii\base\Model
{
    ...
    public function checkHoliday($now)
    {
        ...
        // 六本松の場合、年末年始休業期間なら1/8に発送予定日を再セットする
        if(self::SCENARIO_TY == $this->scenario) {
            if(in_array($now_date, ['2020-01-01'])) {
                $now_date = '2020-01-08 18:00:00';
            } else if(in_array($now_date, ['2019-11-06'])) {
// 臨時休業対応。2019-11-06が休みのため、同日がセットされそうなら、翌週にセットし直す
                $now_date = '2019-11-13 18:00:00';
            } else {
                return $now;
            }

        // それ以外（熱海）の場合は1/6に再セット
        } else {
            if(strtotime('2019-12-28') <= $now && strtotime('2020-01-06') > $now) {
                $now_date = '2020-01-06 09:00';
            } else {
                return $now;
            }
        }
    ...
    public function getDateCandidates()
    {
        if(isset($this->osechi_date))
            $this->now = $this->osechi_date;

        // ticket:677　
        $days  = $this->zipModel->getDays($this->scenario);
        $_24h  = 60 * 60 * 24;
        $start = $this->zipModel->getMinDelivTimeStampYamato($this->now);
        if(self::SCENARIO_TY != $this->scenario) {
            // #375 臨時休業対応
            ...
            // TODO: #377 2019年末年始対応
            if($this->now > strtotime('2019-12-27 12:00:00') && strtotime('2020-01-06 12:00:00') > $this->now) {
                $min_deliv_days = strtotime(date('Y-m-d',$start)) - strtotime(date('Y-m-d',$this->now))/$_24h;
                $data = $this->zipModel->yamato_22; // 佐川が実装されたらここを変更しないといけない。ヤマト決め打ち 2019/10/28
                $min_deliv_days = floor($data / 10);
                // 臨時休業など特定の条件では未適用とする
               if(strtotime(date('Y-m-d',time())) < strtotime('2019-12-28')) {
                   ...
                   $start = strtotime('2020-01-06') + $min_deliv_days * $_24h;
               }
            }
...
<font color="red">2019年12月25日(水)</font>　※12/23締め切り分<br />
<br />
■年始初回出荷<br />
<font color="blue">2020年01月08日(水)</font>　※1月1日（水）出荷はお休み</strong>
</p>
                    </font>
                </div>
            </div>';
...
皆様には大変ご迷惑おかけいたしますが、ご了承いただきますようお願い申し上げます。<br /><br />
<strong>■年内年始休業日</strong><br />
<font color="red">2019年12月28日(土)〜2020年1月5日(日)</font>
</p>
                    </font>
                </div>
            </div>';
