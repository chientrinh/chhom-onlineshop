...
class Invoice extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['customer_id', 'target_date', 'due_purchase', 'due_commission', 'due_pointing', 'due_total'], 'required'],
            [['company_id', 'customer_id', 'created_by', 'updated_by', 'due_purchase', 'due_commission', 'due_pointing', 'due_total'], 'integer'],
    ...
    public function computeTax()
    {
        ...
        foreach($this->getPurchases()->all() as $purchase){
            foreach($purchase->items as $item) {
               ...
               // isReducedTaxで毎回判定せず、レコードのtax_rateで判定する
               if($item->tax_rate == \common\models\Tax::findOne(2)->getRate()){               
