 ...
 * $Id: Purchase.php 4248 2020-04-24 16:29:45Z mori $
 *
...
class Purchase extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        ...
        $rules = [
            [['subtotal','tax','tax10_price', 'tax8_price', 'taxHP_price', 'total_charge'], 'default', 'value'=>0],
...
//            ['point_consume', 'compare','compareAttribute'=>'subtotal', 'operator'=>'<=','message'=>"ポイント値引きが小計を超えています"],
            ...
            [['is_agency','agencies', 'tax_total', 'reduced_tax_total'], 'safe'],
        ];
    ...
    public function compute()
    {
        ...
        foreach ($this->items as $index => $item) {
            ...
            if('echom-frontend' == Yii::$app->id) {
                ...
                if($item->model->category_id == \common\models\Category::LIVE) {
                    $price = isset($grade->price) ? $grade->price : 0;
                    $tax   = isset($grade->tax) ? $grade->tax : 0;

                    if(Yii::$app->session->get('companion_item') == $item->model->product_id){
                        ...
                        if(Yii::$app->session->get('companion_tax'))
                            $tax = Yii::$app->session->get('companion_tax');
                            $item->unit_tax  = $tax;
                    }
                ...
                $taxed_price = ($price + $tax) * $item->quantity;
                $tax10_price += $taxed_price;
                $tax_total += ($tax * $item->quantity);
                // 以降は通常処理
            } else {
...
//var_dump($item->name, $item->unit_price, $item->getUnitTax(), $taxed_price);
                ...
                if($this->create_date && strtotime($this->create_date) <= \common\models\Tax::newDate()) {
                    $item->tax_rate = \common\models\Tax::findOne(2)->getRate();
                    ...
                    if($item->isReducedTax()) {
                        ...
                        $item->tax_rate = \common\models\Tax::findOne(1)->getRate();
                ...
                if($item->isReducedTax()) {
                    $tax8_point += $item->pointTotal;
                } else {
                    $tax10_point += $item->pointTotal;
                }
        ...
        if('echom-frontend' == Yii::$app->id) {
            ...
            $this->total_charge = $this->tax10_price
                                + $this->postage
                                + $this->handling;

            if($this->total_charge < $this->discount)
                $this->discount = $this->total_charge;
                
            $this->total_charge = $this->total_charge - $this->discount;
...
/*
    ...
    {
        return is_array($this->note) ? $this->note : explode(" ", $this->note);
    }

    public function setLiveNotes($val)
    {
        $this->live_notes = $val;
    }
*/
    ...
    {
//      if(($this->status < PurchaseStatus::PKEY_DONE) && $this->shipped && $this->paid)
        if($this->status < PurchaseStatus::PKEY_DONE)
        {
            if($this->shipped)
            {
    ...
    public function validatePayment($attr, $param)
    {
        ...
        if(Payment::PKEY_DIRECT_DEBIT != $this->payment_id || $this->agent_id)
            return true;
