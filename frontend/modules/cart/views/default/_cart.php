        ...
        <div class="row">
            ...
            <?php
                if($cart_idx == Company::PKEY_TY) {
                    ...
                    if(date('Y-m-d',$sample->now) == '2020-01-08' && strtotime('2019-12-31') > time()) {
                       echo $sample->getHolidayMessage();
                    }
// 一括発送カートはcart_idx : 0
                } else if($cart_idx == 0) {
                    $sample = new \common\models\DeliveryDateTimeForm(['company_id'=>$cart_idx]);
// 2019-12-14 00:00 〜 2019-12-20 12:00:00 まで表示する
                    if(strtotime('2019-12-27 00:00:00') <= time() && strtotime('2020-01-05 12:00:00') > time()) {
                       echo $sample->getHolidayMessage();
                    }
                }
            ?>
...
セール品以外の商品にはポイントは付きます。<br /><br />";}?><?= $model->purchase->payment->name ?><?php if ($model->purchase->company_id == Company::PKEY_TROSE){ echo "（全国一律７００円：時間指定不可）";} ?></p>

                <?php if ($model->purchase->company_id == Company::PKEY_TROSE) : ?>
                    <small><p><strong>A ヤマト便代引き（送料＋代引き330円：時間指定可）</strong><br />例）関東 送料￥880 + ￥330 = ￥1210<p><strong>B ゆうちょ振込（５日以内にお振込可能な方のみ対応）</strong><br>連絡無くお振込みの無い場合は、７日目にメール便代引きで発送<p>A・Bご希望の方は備考欄にご記入下さい。</small>
                <?php endif ?>
                <hr>
                <h5>お届け日時
                ...
                <p>
                    <?php if($cart_idx == Company::PKEY_TY): ?>
                        ...
                        <?php } ?>
                           <?= Yii::$app->formatter->asDate($sample->now, 'php:Y年m月d日(D)') ?> に発送予定
                    <?php endif ?>
                </p>                   
